...
window.DASHBOARD = {
  ...
  global: {
    ...
    notes: [
      ...
      "🔴 **更正：前兩版的黃金數字是錯的，而且我用它寫了一整段判斷。** v4.0 寫 8/24 黃金 4,728.90（+2.27%）、v4.1 寫 8/25 為 4,724.70（+1.81%）並據此寫出「唯一不合群的訊號是黃金，連三日漲逾 6%，避險需求沒有解除」。**正確數字是 8/24 收 4,640.80（+0.36%）、8/25 收 4,638.10（-0.06%）**，8/21 到 8/25 累計僅 **+0.30%**——黃金那幾天基本是持平，不是狂漲。原因：抓取用的 `GC=F` 是**連續近月合約**，換月時整條序列的價格水準會位移（本次約 88 點），而我的日變動是拿換月前後兩根 K 棒相減，把**換約價差當成了漲跌**。那段「賣供給中斷的溢價、買地緣衝突的保險」的推論建立在不存在的走勢上，**整段撤回**。腳本待修：黃金應改抓現貨 XAUUSD 而非期貨連續合約。",
